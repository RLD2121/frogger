...
Enemy.prototype.update = function(dt) {
    ...
    if (this.x > 505) {
        this.x = this.getRandomArbitrary(-1000, -100);
    }
};
...
Player.prototype.checkCollision = function() {
    for (var i = 0; i < allEnemies.length; i++) {
        if ((this.x + 16) < allEnemies[i].x + allEnemies[i].width &&
            (this.x + 16) + (this.width + 63) > allEnemies[i].x &&
            (this.y + 63) < (allEnemies[i].y + 77) + allEnemies[i].height &&
            this.height + (this.y + 63) > (allEnemies[i].y + 77)) {
